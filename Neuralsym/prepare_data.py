...
def gen_prod_fps(args):
    ...
    for phase in ['train', 'valid', 'test']:
        ...
        products_list, reactants_list = get_dataset(phase, True)
...
if not os.path.exists(f"{args.data_folder}/{args.output_file_prefix}_prod_fps_valid.npz"):
    # ~2 min on 40k train prod_smi on 16 cores for 32681-dim
    gen_prod_fps(args)
if not os.path.exists(f"{args.data_folder}/{args.output_file_prefix}_to_{args.final_fp_size}_prod_fps_valid.npz"):
...
if not os.path.exists(f"{args.data_folder}/{args.output_file_prefix}_csv_train.csv"):
