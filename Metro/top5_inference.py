...
def get_output_probs(products, res, max_depth, max_length):
    products_ids, products_mask, reactants_ids, reactants_mask, memory_mask, _ = convert_symbols_to_inputs([products], [res], max_depth, max_length)
     # To Tensor
    products_ids = torch.LongTensor(products_ids).to(device)
    reactants_ids = torch.LongTensor(reactants_ids).to(device)

    products_mask = torch.FloatTensor(products_mask).to(device)
    reactants_mask = torch.FloatTensor(reactants_mask).to(device)
    memory_mask = torch.FloatTensor(memory_mask).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_depth', type=int, default=13, help='The max depth of a synthesis route.')
    ...
    for epoch in trange(0, len(test_set)):
        ...
        while True:
            ...
            for item in queue:
                ...
                if depth > max_depth:
                    continue
                for expansion_solution in get_beam(first_route, args.beam):
