...
from preprocess import convert_symbols_to_inputs, get_vocab_size, get_char_to_ix, get_ix_to_char
...
def get_output_probs(product, res):
    test_products_ids, test_products_mask, test_res_ids, test_res_mask, _ = convert_symbols_to_inputs([product], [res], args.max_length)
    # To Tensor
    test_mutual_mask = get_mutual_mask([torch.FloatTensor(test_res_mask), torch.FloatTensor(test_products_mask)]).to(device)
    test_products_mask = get_products_mask(torch.FloatTensor(test_products_mask)).to(device)
    test_res_mask = get_reactants_mask(torch.FloatTensor(test_res_mask)).to(device)

    test_products_ids = torch.LongTensor(test_products_ids).to(device)
    test_res_ids = torch.LongTensor(test_res_ids).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--beam', type=int, default=5, help='Beams size. Default 5. Must be 1 meaning greedy search or greater or equal 5.')
    ...
    for epoch in trange(0, len(test_set)):
        ...
        while True:
            ...
            for item in queue:
                ...
                if depth > max_depth:
                    continue
                for expansion_solution in get_beam(first_route[-1], args.beam):
